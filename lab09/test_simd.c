...
int main(int argc, char *argv[]) {
  ...
  printf("Starting randomized SIMD parallelized sum.\n");
  start = clock();
  simd = sum_simd_parallel(vals);
  end = clock();
  printf("Sum: %lld\n", simd);
  clock_t simdpt = end - start;
  ...
  if (simdu != reference) {
    printf("Test Failed! SIMD_UNROLLED sum %lld doesn't match reference sum "
           "%lld!\n",
           simdu, reference);
    success = 1;
  }
  ...
  if (!success) {
    printf("All tests Passed! Correct values were produced, and speedups were "
           "achieved!\n");
    return 0;
  } else {
    return 1;
