#include "numc.h"
#include </home/crvena/anaconda3/envs/numc/include/python3.6m/structmember.h>
...
#define MATRIX_BINARY_OP(op)                                                  \
  if (PyObject_IsInstance (args, (PyObject *)&Matrix61cType))                 \
    {                                                                         \
      ...
      matrix *result = NULL;                                                  \
      Matrix61c *new_object = NULL;                                           \
      int alloc_failed                                                        \
          = allocate_matrix (&result, self->mat->rows, self->mat->cols);      \
                                                                              \
      switch (alloc_failed)                                                   \
        {                                                                     \
        case -1:                                                              \
      ...
      new_object = (Matrix61c *)Matrix61cType.tp_alloc (&Matrix61cType, 0);   \
      if (!new_object)                                                        \
        return NULL;                                                          \
      new_object->mat = result;                                               \
      new_object->shape = get_shape (result->rows, result->cols);             \
      return (PyObject *)new_object;                                          \
    }                                                                         \
  else                                                                        \
    {                                                                         \
      PyErr_SetString (PyExc_TypeError,                                       \
                       "Argument must of type numc.Matrix!");                 \
      return NULL;                                                            \
    }

#define MATRIX_UNARY_OP(op)                                                   \
  matrix *result = NULL;                                                      \
  Matrix61c *new_object = NULL;                                               \
  int alloc_failed                                                            \
      = allocate_matrix (&result, self->mat->rows, self->mat->cols);          \
 ...
 */
int
init_rand (PyObject *self, int rows, int cols, unsigned int seed, double low,
           double high)
{
  matrix *new_mat;
 ...
 */
int
init_fill (PyObject *self, int rows, int cols, double val)
{
  matrix *new_mat;
  int alloc_failed = allocate_matrix (&new_mat, rows, cols);
  if (alloc_failed)
 ...
 */
int
init_1d (PyObject *self, int rows, int cols, PyObject *lst)
{
  if (rows * cols != PyList_Size (lst))
    {
      PyErr_SetString (PyExc_ValueError,
                       "Incorrect number of elements in list");
      return -1;
    }
  matrix *new_mat;
  int alloc_failed = allocate_matrix (&new_mat, rows, cols);
 ...
 */
int
init_2d (PyObject *self, PyObject *lst)
{
 ...
 */
int
Matrix61c_init (PyObject *self, PyObject *args, PyObject *kwds)
{
      ...
      if (PyArg_UnpackTuple (args, "args", 2, 2, &rows, &cols))
        {
          if (rows && cols && PyLong_Check (rows) && PyLong_Check (cols))
            {
              return init_rand (self, PyLong_AsLong (rows),
                                PyLong_AsLong (cols), unsigned_seed,
      ...
      if (arg1 && arg2 && arg3 && PyLong_Check (arg1) && PyLong_Check (arg2)
          && (PyLong_Check (arg3) || PyFloat_Check (arg3)))
        {
          if (PyLong_Check (arg3))
            {
              return init_fill (self, PyLong_AsLong (arg1),
                                PyLong_AsLong (arg2), PyLong_AsLong (arg3));
            }
          else
               ...
               && PyLong_Check (arg2) && PyList_Check (arg3))
        {
          ...
          return init_1d (self, PyLong_AsLong (arg1), PyLong_AsLong (arg2),
                          arg3);
        }
      ...
      for (int i = 0; i < rows; i++)
        {
          for (int j = 0; j < cols; j++)
            {
              PyList_SetItem (py_lst, count,
                              PyFloat_FromDouble (get (self->mat, i, j)));
              count++;
            }
        }
    }
...
PyObject *
Matrix61c_class_to_list (Matrix61c *self, PyObject *args)
{
  PyObject *mat = NULL;
 ...
 */
PyObject *
Matrix61c_add (Matrix61c *self, PyObject *args){ MATRIX_BINARY_OP (add) }
...
//         }
//
//       int add_failed = add_matrix (result, self->mat, mat61c->mat);
//       switch (add_failed)
//         {
//         case -1:
//           PyErr_SetString (PyExc_ValueError, "Matrix shape mismatch");
...
//           return NULL;
//         }
//       new_object = (Matrix61c *)Matrix61cType.tp_alloc (&Matrix61cType,
//       0); if (!new_object)
//         return NULL;
//       new_object->mat = result;
//       new_object->shape = get_shape (result->rows, result->cols);
...
//   else
//     {
//       PyErr_SetString (PyExc_TypeError, "Argument must of type
//       numc.Matrix!"); return NULL;
//     }
// }

/*
 * Substract the second numc.Matrix (Matrix61c) object from the first one.
 * The first operand is self, and the second operand can be obtained by
 * casting `args`.
 */
PyObject *Matrix61c_sub (Matrix61c *self,
                         PyObject *args){ MATRIX_BINARY_OP (sub) }
 ...
 */
PyObject *
Matrix61c_neg (Matrix61c *self){ MATRIX_UNARY_OP (neg) }

/*
 * Take the element-wise absolute value of this numc.Matrix.
 */
PyObject *Matrix61c_abs (Matrix61c *self){ MATRIX_UNARY_OP (abs) }
...
Matrix61c_set_value (Matrix61c *self, PyObject *args)
{
  unsigned row, col;
  double val;
  if (PyArg_ParseTuple (args, "IId:set", &row, &col, &val))
    {
      if (row >= 0 && col >= 0 && row < self->mat->rows
          && col < self->mat->cols)
        {
      ...
      else
        {
          PyErr_Format (PyExc_ValueError,
                        "Cannot set matrix of shape (%d, %d) at (%d, %d)",
                        self->mat->rows, self->mat->cols, row, col);
        }
    }
  else
    {
      PyErr_Format (PyExc_SyntaxError, "Expected 3 arguments, found %d",
                    PyTuple_Size (args));
    }
  Py_RETURN_NONE;
}
  ...
  if (PyArg_ParseTuple (args, "II:get", &row, &col))
    {
      if (row >= 0 && col >= 0 && row < self->mat->rows
        ...
        {
          PyErr_Format (PyExc_ValueError,
                        "Cannot get from matrix of shape (%d, %d) at (%d, %d)",
                        self->mat->rows, self->mat->cols, row, col);
        }
    }
  else
    {
      PyErr_Format (PyExc_SyntaxError, "Expected 2 arguments, found %d",
                    PyTuple_Size (args));
    }
  Py_RETURN_NONE;
}
...
PyMethodDef Matrix61c_methods[] = {
  ...
  { "get", (PyCFunction)Matrix61c_get_value, METH_VARARGS,
    "Return the value of matrix at (row, col)" },
  { "set", (PyCFunction)Matrix61c_set_value, METH_VARARGS,
    "Set the value of matrix at (row, col) to be (value)" },
  { NULL, NULL, 0, NULL }
};
 ...
 */
PyObject *
Matrix61c_subscript (Matrix61c *self, PyObject *key)
{
  /* TODO: YOUR CODE HERE */
}
 ...
 */
int
Matrix61c_set_subscript (Matrix61c *self, PyObject *key, PyObject *v)
{
  /* TODO: YOUR CODE HERE */
}
...
struct PyModuleDef numcmodule
    = { PyModuleDef_HEAD_INIT, "numc", "Numc matrix operations", -1,
